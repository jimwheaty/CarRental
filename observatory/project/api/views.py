...
def funcLogin(request):
    ...
    if method == "POST":    
        ...
        return JsonResponse({'X-OBSERVATORY-AUTH': token})
...
def funcSignup(request):
    ...
    if method == "POST":    
        ...
        salt = bcrypt.gensalt()
        hashed_given_password = bcrypt.hashpw(given_password, salt)
        ...
        token = ''.join(random.choices(string.ascii_uppercase + string.digits, k=10))
        cursor = connection.cursor()
        cursor.execute("INSERT INTO Volunteer(Volunteer_id, Username, Password, Token) VALUES (NULL, \"" + username + "\"" + ", \"" + hashed_given_password + "\", \"" + token + "\");")
        return JsonResponse({'X-OBSERVATORY-AUTH ': token})
    return HttpResponseBadRequest("400 Bad Request: Unknown command") 
